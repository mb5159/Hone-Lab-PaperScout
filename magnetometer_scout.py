...
def send_sendgrid(subject: str, html: str, text: str, to_email: str) -> None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            status = resp.status
            print(f" ✅ SendGrid: {status} — sent to {to_email}.")
...
def main(test_mode: bool = False, force_lookback: int | None = None) -> None:
    ...
    if test_mode:
        ...
        send_sendgrid(subject, html, text, CONFIG["to_email"])
